package productsaleinterface_refund_teacher;
...
class Buyer{
	...
	/*
	 * Product타입만 저장하는 ArrayList객체 생성
	 * 문법적으로 new ArrayList<>안의 Product 생략가능
	 * 크기가 자동으로 커짐. 추가와 삭제에 대한 메서드가 있음
	 * 
	 * List 인터페이스 : 부모역할가능 - index(o) -> 순서가있다, 중복허용
	 * -ArrayList 클래스 : 끝에 추가하거나 삭제하면 사용이 빠름 (=차례대로 추가,삭제), 중간에 추가나 삭제하면 처리시간이 오래걸림
	 * -LinkedList 클래스 : 중간에 추가나 삭제가 용이하여 처리시간이 빠름
	 * -Vector 클래스 : 멀티쓰레드에서 사용 싱글쓰레드에서 사용가능하나 느림. 나머지특징은 ArrayList와 같음
	 */
	...
	void refund(Product p){
		//해당제품을 목록에서 찾아서 있으면 제거후 true, 없으면 false
		if(item.remove(p)) {
			this.money += p.price;
			this.bonusPoint -= p.bonusPoint;
	...
	void summary(){
		...
		for (int i = 0 ; i < item.size() ; i++) {
			...
			case "AUDIO" : cntAud++; totalcnt++; break; // 마지막 break;생략가능
			}
		}
		
		if(item.size() != 0) {
			System.out.println("==========현황==========");
			//배열에 담긴 값들 사이 , 찍고 마지막 , 제거
			//방법1. toString메서드 재정의 후
			//itemList = itemList + ((i==0)? ""+p : ","+p);
			//방법2.  문자열 사이에 구분자(,)를 두어 문자열 결합
			itemList=String.join(", ", item.toString());
...
public class ProductInterfaceArrayList implements ProductInter{
